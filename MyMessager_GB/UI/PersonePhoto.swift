...
@IBDesignable class PersonePhoto: UIView {
    ...
    @IBInspectable var shadowRadius: Float = 5{
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    private func createMaskFromImage() -> CAShapeLayer{
        ...
        let starPath = UIBezierPath(ovalIn: CGRect(x: bounds.minX + CGFloat(shadowRadius/2), y: bounds.minY + CGFloat(shadowRadius/2), width: bounds.width - CGFloat(shadowRadius), height: bounds.height - CGFloat(shadowRadius)))
